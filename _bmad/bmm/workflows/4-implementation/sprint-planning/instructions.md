...
<step n="6" goal="Sync to Beads (automatic task tracking)" optional="true">
  ...
  <action>Check if "sprint-planning" is in sync_triggers list</action>
  <check if="sprint-planning not in sync_triggers">
    <note>Beads sync not configured for sprint-planning workflow - skipping</note>
    <action>Exit workflow</action>
  </check>
