import { g as getWidgetApiBase } from "./loader-Cgldy3pq.js";
...
function isWebSocketEnvironmentSupported() {
  ...
  if (apiBase.includes("zrok.io")) {
    console.warn("[WS] Zrok tunnel detected - WebSocket not supported, using polling instead");
    return false;
  }
  return true;
}
function connectWidgetWebSocket(sessionId, options = {}) {
  ...
  if (!isWebSocketEnvironmentSupported()) {
    console.warn("[WS] WebSocket not supported in this environment, skipping connection");
    onStatusChange == null ? void 0 : onStatusChange("error");
    return () => {
      console.warn("[WS] Cleanup - no connection to close");
    };
  }
  let ws = null;
