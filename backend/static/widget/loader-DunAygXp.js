...
function WidgetProvider({ children, merchantId }) {
  ...
  const {
    ...
    () => ({
      createSession: async () => {
        const { widgetClient } = await import("./widgetClient-BISbVPd3.js");
...
Your cart now has ${updatedCart.itemCount} ${itemWord} totaling $${updatedCart.total.toFixed(2)}.`,
  ...
  reactExports.useEffect(() => {
    ...
    const connectSSE = async () => {
      const { connectToWidgetSSE, isSSESupported } = await import("./widgetSSEClient-BnBxVtxp.js");
...
const ChatWindow$2 = reactExports.lazy(() => import("./ChatWindow-CiaSim_l.js"));
