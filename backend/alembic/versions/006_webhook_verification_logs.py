...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "platform",
            sa.Enum("facebook", "shopify", name="verification_platform", create_type=False),
            nullable=False,
        ),
        sa.Column(
            "test_type",
            sa.Enum(
                "status_check", "test_webhook", "resubscribe", name="test_type", create_type=False
            ),
            nullable=False,
        ),
        sa.Column(
            "status",
            sa.Enum("pending", "success", "failed", name="verification_status", create_type=False),
            nullable=False,
    ...
    op.create_index(
        "ix_webhook_verification_logs_merchant_id", "webhook_verification_logs", ["merchant_id"]
    )
    op.create_index(
        "ix_webhook_verification_logs_platform", "webhook_verification_logs", ["platform"]
    )

    # Add last_webhook_verified_at column to facebook_integrations
    op.add_column(
        "facebook_integrations", sa.Column("last_webhook_verified_at", sa.DateTime(), nullable=True)
    )

    # Add last_webhook_verified_at column to shopify_integrations
    op.add_column(
        "shopify_integrations", sa.Column("last_webhook_verified_at", sa.DateTime(), nullable=True)
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_webhook_verification_logs_merchant_id", table_name="webhook_verification_logs"
    )
