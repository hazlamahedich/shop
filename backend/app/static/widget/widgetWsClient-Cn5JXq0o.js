  ...
  const connect = () => {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        console.warn("[WS] About to call onMessage, type:", parsed.type, "handler exists:", !!onMessage);
        if (onMessage) {
          console.warn("[WS] Calling onMessage with:", parsed);
          onMessage(parsed);
          console.warn("[WS] onMessage call completed");
        } else {
          console.warn("[WS] onMessage callback is undefined!");
        }
      } catch (e) {
