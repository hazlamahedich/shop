...
export function DeploymentWizard(): React.ReactElement {
  ...
  const handleDeploy = async () => {
    if (!platform) return;
    await startDeployment(platform);
  };
        ...
        <CardContent className="space-y-6">
          ...
          {(isDeploying || isComplete || hasFailed || isCancelled) && (
            <div className="space-y-4">
              ...
              {logs.length > 0 && (
                <div>
                  <h3 className="text-sm font-medium text-slate-700 mb-2" id="deployment-logs-title">
                    Deployment Logs
                  </h3>
              ...
              {isComplete && merchantKey && (
                <Alert variant="default" title="Deployment Successful!">
                  <p className="mb-3">
                    ...
                    <code className="px-1 py-0.5 bg-slate-100 rounded text-sm">
                      {merchantKey}
                    </code>
                  </p>
                  <p className="font-medium mb-2">Next Steps:</p>
                  <ol className="list-decimal list-inside space-y-1 text-sm" aria-label="Next steps after deployment">
