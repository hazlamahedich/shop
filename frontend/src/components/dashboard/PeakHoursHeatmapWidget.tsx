    ...
    >
      ...
      <div className="p-5">
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-amber-500/10 text-amber-400 ring-4 ring-amber-500/20">
            <Clock size={18} />
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="flex items-center justify-center py-8">
            <p className="text-sm text-white/60">Unable to load peak hours data</p>
          </div>
        ) : hourlyBreakdown.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8">
            <Users size={32} className="text-white/40 mb-2" />
            <p className="text-sm text-white/60">No conversation data yet</p>
              ...
              <div className="min-w-[600px]">
                <div className="flex gap-0.5 mb-1">
                  ...
                  {HOURS.slice(6, 22).map((hour) => (
                    <div
                      key={hour}
                      className="w-6 text-center text-[10px] text-white/60"
                    >
                      {formatHour(hour)}
                    </div>
                  ))}
                ...
                {DAYS.map((day, dayIndex) => (
                  <div key={day} className="flex gap-0.5 mb-0.5">
                    <div className="w-10 text-xs text-white/60 flex items-center">
                      {day}
                    </div>
                    {HOURS.slice(6, 22).map((hour) => {
            ...
            <div className="mt-4 pt-4 border-t border-white/10">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div>
                    <p className="text-xs text-white/60">Busiest Day</p>
