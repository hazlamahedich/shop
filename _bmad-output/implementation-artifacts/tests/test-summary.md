...
# Run with auth tokens (for full API coverage)
TEST_AUTH_TOKEN=xxx TEST_CSRF_TOKEN=xxx npx playwright test tests/api/story-4-9-hybrid-mode.spec.ts --project=api
```

## Next Steps

- [ ] Set `TEST_AUTH_TOKEN` and `TEST_CSRF_TOKEN` in CI for full API test coverage
- [ ] Add tests to CI pipeline
- [ ] Add edge case tests for timeout scenarios
